...
def no_fixed(dv):
    df2009, df2010 = map(pd.read_csv,
                         [opath.join(dpath['analysis'], 'ap-dropoff-2009.csv'),
                          opath.join(dpath['analysis'], 'ap-dropoff-2010.csv')])
    ofpath = opath.join(dpath['dropoffAP_tTest'], 'no_fixed_%s.csv' % dv)
    with open(ofpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
        writer.writerow(['dow'] + stats)
    num2009, num2010 = map(len, [df2009, df2010])
    ratio2009 = sum(df2009['J']) / float(num2009)
    ratio2010 = sum(df2010['J']) / float(num2010)
...
def dow_fixed(dv):
    df2009, df2010 = map(pd.read_csv,
                         [opath.join(dpath['analysis'], 'ap-dropoff-2009.csv'),
                          opath.join(dpath['analysis'], 'ap-dropoff-2010.csv')])
    ofpath = opath.join(dpath['dropoffAP_tTest'], 'dow_fixed_%s.csv' % dv)
    with open(ofpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
        writer.writerow(['dow'] + stats)
    ...
    for i, w in enumerate(dows):
        ...
        ratio2009 = sum(dow_df2009['J']) / float(num2009)
        ratio2010 = sum(dow_df2010['J']) / float(num2010)
...
def month_fixed(dv):
    df2009, df2010 = map(pd.read_csv,
                         [opath.join(dpath['analysis'], 'ap-dropoff-2009.csv'),
                          opath.join(dpath['analysis'], 'ap-dropoff-2010.csv')])
    ofpath = opath.join(dpath['dropoffAP_tTest'], 'month_fixed_%s.csv' % dv)
    with open(ofpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
    ...
    for i, m in enumerate(months):
        ...
        ratio2009 = sum(month_df2009['J']) / float(num2009)
        ratio2010 = sum(month_df2010['J']) / float(num2010)
...
def hour_fixed(dv):
    df2009, df2010 = map(pd.read_csv,
                         [opath.join(dpath['analysis'], 'ap-dropoff-2009.csv'),
                          opath.join(dpath['analysis'], 'ap-dropoff-2010.csv')])
    ofpath = opath.join(dpath['dropoffAP_tTest'], 'hour_fixed_%s.csv' % dv)
    with open(ofpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
    ...
    for i, h in enumerate(hours):
        ...
        ratio2009 = sum(hour_df2009['J']) / float(num2009)
        ratio2010 = sum(hour_df2010['J']) / float(num2010)
...
def driver_fixed(dv):
    df2009, df2010 = map(pd.read_csv,
                         [opath.join(dpath['analysis'], 'ap-dropoff-2009.csv'),
                          opath.join(dpath['analysis'], 'ap-dropoff-2010.csv')])
    ofpath = opath.join(dpath['dropoffAP_tTest'], 'driver_fixed_%s.csv' % dv)
    with open(ofpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
    ...
    for did in drivers:
        ...
        ratio2009 = sum(did_df2009['J']) / float(num2009)
        ratio2010 = sum(did_df2010['J']) / float(num2010)
...
def month_driver_fixed(dv):
    df2009, df2010 = map(pd.read_csv,
                         [opath.join(dpath['analysis'], 'ap-dropoff-2009.csv'),
                          opath.join(dpath['analysis'], 'ap-dropoff-2010.csv')])
    months = list(set(df2009['month']).intersection(set(df2010['month'])))
    months.sort()
    ofpath = opath.join(dpath['dropoffAP_tTest'], 'month_driver_fixed_%s.csv' % dv)
    with open(ofpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
    ...
    for m in months:
        ...
        for did in drivers:
            ...
            ratio2009 = sum(month_did_df2009['J']) / float(num2009)
            ratio2010 = sum(month_did_df2010['J']) / float(num2010)
